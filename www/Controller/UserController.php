...
class UserController {
            ...
            if( !empty($_POST)){
                ...
                if (empty($result)) {
                   ...
                   if ( Verificator::checkIfOnlyLetters($_POST['firstname']) && Verificator::checkIfOnlyLetters($_POST['lastname']))
                   {
                        ...
                        $mailtest->sendMail('register', $user->getEmail(), $user->getFirstname(),"http://localhost/activation/".$user->getToken());
    ...
    {   
        $code = explode("/", $_SERVER["REQUEST_URI"])[2];

        if(ctype_alnum($code)){ //Si le code est en alphanumerique
            $userSecurity = new UserSecurity();
            echo $userSecurity->validateAccount($code); //On check si un compte possède ce token
            header('Location: /');
        }else{
            header('Location: /');
        }
